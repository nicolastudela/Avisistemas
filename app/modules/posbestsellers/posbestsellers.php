...
class posbestsellers extends Module
{
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			|| !Configuration::updateValue('PRODUCT_ROW',4)
	...
	{
		if (!$this->isCached('posbestsellers.tpl', $this->getCacheId('posbestsellers')))
		{
		...
		return $this->display(__FILE__, 'posbestsellers.tpl', $this->getCacheId('posbestsellers'));
	}
	...
	{
		if (!$this->isCached('posbestsellers_product.tpl', $this->getCacheId('posbestsellers-col')))
		{
			if (!isset(posbestsellers::$cache_best_sellers))
				posbestsellers::$cache_best_sellers = $this->getBestSellers($params);
			$this->smarty->assign(array(
