...
<div id="product-images" class="panel product-tab">
		...
		<tr id="image_id">
			...
			<td class="rotator">
				<a href="#">
					...
					<i class="icon-check-empty icon-2x rotatored"></i>
				</a>
			</td>
			<td>
	...
	<script type="text/javascript">
		...
		function imageLine(id, path, position, cover, shops, legend)
		{
			...
			line = line.replace(/icon-check-empty icon-2x covered/g, cover);
		...
		$(document).ready(function(){
			...
			{foreach from=$images item=image}
				...
				if (assoc != {literal}"{"{/literal})
				{
					...
					assoc = false;
				imageLine({$image->id}, "{$image->getExistingImgPath()}", {$image->position}, "{if $image->cover}icon-check-sign icon-2x covered{else}icon-check-empty icon-2x covered{/if}", assoc, "{$image->legend[$default_language]|@addcslashes:'\"'}");
			{/foreach}
			{literal}
			...
			//$('#link-Images').bind('click', function() {
					$.ajax({
						...
						success: function(json) { 
							...
							console.log(td);
							var rotator = $(td).find('.rotatored').removeClass('icon-check-empty').addClass('icon-check-sign');
						},
			...
			$('.rotatored').die().live('click',function(e){
				e.preventDefault();
				var id_product = {/literal}{$id_product}{literal}
				id = $(this).parent().parent().parent().attr('id');
				...
				if($(this).hasClass('icon-check-sign')){
					...
					$("#imageList .rotator i").each( function(i){
						$(this).removeClass('icon-check-sign').addClass('icon-check-empty');
					});
					$(this).removeClass('icon-check-empty').addClass('icon-check-sign');
				}				
