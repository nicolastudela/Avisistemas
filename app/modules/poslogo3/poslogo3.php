...
// Loading Models
require_once(_PS_MODULE_DIR_ . 'poslogo3/models/Logo3.php');
class poslogo3 extends Module {
    private $_html = '';
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Pos Logo 3');
    ...
    public function install()
	{
		...
		foreach (Language::getLanguages() as $language)
            $tab->name[$language['id_lang']] = $this->l('Manage Logo 3');
		$tab->class_name = 'AdminPosLogo3';
		...
		// Set some defaults
		 ...
		 $this->registerHook('brandSlider3')&&
		...
		Configuration::deleteByName('poslogo3');

		// Uninstall Tabs
		
                $tab = new Tab((int)Tab::getIdFromClassName('AdminPosLogo3'));
		$tab->delete();
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitPostLogo3'))
		{			
        ...
        public function getLogo() { 
                        ...
                        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'pos_logo3` ps LEFT JOIN `' . _DB_PREFIX_ . 'pos_logo3_shop`  s ON ps.id_pos_logo3 = s.id_pos_logo3 where s.`id_shop` ='.$id_shop.'  ORDER BY `porder` ASC';
                            ...
                            foreach($slides  as $key => $slideArray) {
                                ...
                                $newSlide = array();
                                 foreach($slideArray as $k => $v) {
                                     if($k=='image'){
                                         $v = _PS_BASE_URL_.__PS_BASE_URI__.'img/blocklogo3/'.$slideArray['id_pos_logo3'].'.jpg';
                                     }
    ...
    private function _displayForm() {
        ...
        $this->_html .='
                    ...
                    <input type="submit" name="submitPostLogo3" value="' . $this->l('Update') . '" class="button" />
                     </fieldset>
		</form>';
        return $this->_html;
    }
	...
	function hookBrandSlider3($params) {
		$options = array(
            ...
            $this->context->smarty->assign('logos3', $logos);
            return $this->display(__FILE__, 'logo3.tpl');
	}
	function hookblockPosition3($params) {
        return $this->hookBrandSlider3($params);
    }
