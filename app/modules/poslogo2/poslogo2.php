...
// Loading Models
require_once(_PS_MODULE_DIR_ . 'poslogo2/models/Logo2.php');
class poslogo2 extends Module {
    private $_html = '';
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Pos Logo 2');
    ...
    public function install()
	{
		...
		foreach (Language::getLanguages() as $language)
            $tab->name[$language['id_lang']] = $this->l('Manage Logo 2');
		$tab->class_name = 'AdminPosLogo2';
		...
		// Set some defaults
		 ...
		 $this->registerHook('brandSlider2')&&
		...
		Configuration::deleteByName('poslogo2');

		// Uninstall Tabs
		
                $tab = new Tab((int)Tab::getIdFromClassName('AdminPosLogo2'));
		$tab->delete();
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitPostLogo2'))
		{			
        ...
        public function getLogo() { 
                        ...
                        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'pos_logo2` ps LEFT JOIN `' . _DB_PREFIX_ . 'pos_logo2_shop`  s ON ps.id_pos_logo2 = s.id_pos_logo2 where s.`id_shop` ='.$id_shop.'  ORDER BY `porder` ASC';
                            ...
                            foreach($slides  as $key => $slideArray) {
                                ...
                                $newSlide = array();
                                 foreach($slideArray as $k => $v) {
                                     if($k=='image'){
                                         $v = _PS_BASE_URL_.__PS_BASE_URI__.'img/blocklogo2/'.$slideArray['id_pos_logo2'].'.jpg';
                                     }
    ...
    private function _displayForm() {
        ...
        $this->_html .='
                    ...
                    <input type="submit" name="submitPostLogo2" value="' . $this->l('Update') . '" class="button" />
                     </fieldset>
		</form>';
        return $this->_html;
    }
	...
	function hookBrandSlider2($params) {
		$options = array(
            ...
            $this->context->smarty->assign('logos2', $logos);
            return $this->display(__FILE__, 'logo2.tpl');
	}
	function hookblockPosition3($params) {
        return $this->hookBrandSlider2($params);
    }
